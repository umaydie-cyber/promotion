...
export default class BattleScene extends Phaser.Scene {
    ...
    private useCardNoTarget(indexInHand: number) {
        ...
        // 归元等：强制结束回合
        if (this.forceEndTurnRequested) {
            this.forceEndTurnRequested = false;
            this.endPlayerTurn();
            return;
        }
    ...
    // ======== 伤害结算 ========
    private dealDamageToEnemyWithSwordIntent(enemyId: EnemyId, base: number) {
        const total = base + this.playerSwordIntent;
        this.applyDamageToEnemy(enemyId, total);
        return total;
    }

    private dealDamageToAllEnemiesWithSwordIntent(base: number) {
        for (const e of this.aliveEnemies()) {
            this.dealDamageToEnemyWithSwordIntent(e.id, base);
        }
    }
